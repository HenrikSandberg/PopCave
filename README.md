...
I designed and built my app PopCave from scratch in order to salve the exam assignments. The app presents the top 50 albums of all time according to the site theaudiodb.com as well as giving the user recommences based on which tracks they like by different artists. 

## Third party libraries
I have used third party libraries before when working with iOS code. They have been quite useful, especially coco pod libraries such as AlamoFire. However, libraries adds a complexity to the project too and many can make the app experience worse. I therefor decided to write all code self. This resulted in more code then what would be possible to produce by using third party libraries. However, the controller gotten by handling everything my self is quite valuable. 
...
## Logo
I went for a simple logo with only two colors. The name comes from the idea that this app is a horder of pop music through the ages and the arrow represents an eternal loop, just like a playlist set on repeat. 
...
![Compare light and dark mode][image-2]
...
The last thing to be added to core data is the recommendations. This table is reset and filled every time the user adds or removes a favorite tracks. 
...
A site I have used a lot in order to handle JSON data was [https://jsonformatter.curiousconcept.com][1]. This page helps formatting  the JSON data in order to understanding it’s structure and how to access the data needed. This site made it clear to me how the `Decodable` structs should look and what they should include. 

## Extension
I split as much as possible in the view controller files into there own extension files just to make the code easier to read trough. Other then that I extended Int, String and TimeInterval. There extensions can be found under the model folder in the file Extensions.swift. 
...
The last extension for string is a long series of removal of special symbols when the string is being used as a url. The code is long and not that pretty so it is not included in this fil, however it is accessible in the Extensions.swift file.  
...
![Compare differences in list and list structure][image-4]
...
![Compare differences in seach for artist and album][image-5]
...
![List of favorites with recommendations on top][image-6]
...
![List of all albums made by recommended artist][image-7]
...
[image-1]:	screenshots/size.png "Screen sizes"
[image-2]:	screenshots/darkmode.png "Darkmode"
[image-3]:	screenshots/album.png "Album view"
[image-4]:	screenshots/gridAndList.png "List & Grid view"
[image-5]:	screenshots/search.png "Search view"
[image-6]:	screenshots/favescreen.png "Favorite view"
[image-7]:	screenshots/recommendation.png "Recommendation view"